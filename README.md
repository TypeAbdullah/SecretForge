...
```javascript
const sf = require('secretforgejs');
...
Install globally to use `secretforgejs` anywhere:

```bash
npm i -g secretforgejs

secretforgejs jwt
secretforgejs jwt:refresh --length 256
secretforgejs password --length 40
secretforgejs passphrase --words 6
secretforgejs api:prefixed --prefix sk --environment live
secretforgejs db:postgres --dbName production
secretforgejs oauth:pkce
secretforgejs uuid --count 10
secretforgejs dotenv --appName MyApp
secretforgejs analyze --secret "MyP@ssw0rd!"
secretforgejs --help
```
...
- 🛠️ **Universal Generator**: A single unified API for all your secret needs.
- 💻 **CLI First**: Powerful command-line interface for instant generation.
- 📦 **Library Ready**: Full-featured Node.js API with zero external dependencies.
- 🔒 **Security-Centric**: Built on top of Node's built-in `crypto` module.
